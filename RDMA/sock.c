...
ssize_t
sock_read (int sock_fd, void *buffer, size_t len){
    ssize_t nr, tot_read;
...
ssize_t
sock_write (int sock_fd, void *buffer, size_t len){
...
int
sock_create_connect (char *server_name, char *port){
    struct addrinfo hints;
    struct addrinfo *result, *rp;
    int sock_fd = -1, ret = 0;
...
int
sock_set_qp_info(int sock_fd, struct QPInfo *qp_info){
    int n;
...
int
sock_get_qp_info(int sock_fd, struct QPInfo *qp_info){
    int n;
    struct QPInfo  tmp_qp_info;
